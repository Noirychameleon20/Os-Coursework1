/* ============================================================================
 *  SJF Scheduling — Cross-Platform (Linux & Windows)
 *  Context : Online Examination System
 *  Module  : ST5068CEM | Platforms and Operating Systems
 *
 *  Compile & Run
 *  Linux  : gcc -o sjf sjf.c && ./sjf
 *  Windows: gcc -o sjf.exe sjf.c && .\sjf.exe
 * ============================================================================
 */
...
static void sjf(Task t[], int n) {
    int completed = 0, time = 0;
    while (completed < n) {
        int best = -1, min_burst = INT_MAX;
        for (int i = 0; i < n; i++)
            if (!t[i].done && t[i].arrival <= time && t[i].burst < min_burst) {
                min_burst = t[i].burst; best = i;
            }
...
int main(void) {
    printf("\n");
    printf("============================================================\n");
    printf("  SJF Scheduling  |  Online Examination System\n");
